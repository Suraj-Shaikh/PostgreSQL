CREATE OR REPLACE FUNCTION maharain.fn_calculate_dry_spell(
    start_year INT,
    end_year INT
)
RETURNS TABLE (
    c_district_name text,
    c_tehsil_name text,
    circle_name text,
    circle_aws text,
    latitude numeric,
    longitude numeric,
    is_dry_spell integer,
    start_date date,
    end_date date,
    dry_spell_date text,
    row_count integer,
...
BEGIN
    FOR year IN start_year..end_year LOOP
        sql_query := '
            WITH RECURSIVE dry_spell_cte AS (
                SELECT
                    c_district_name,
                    c_tehsil_name,
                    station,
                ...
                FROM
                    maharain.rainfall_' || year || '
                WHERE  
                    rain_date >= ''' || year || '-06-01''::date AND rain_date <= ''' || year || '-10-31''::date
            ),
                ...
                FROM 
                    (
                        SELECT
                            ...
                            longitude,
                            rain_date,
                            CAST(is_dry_spell AS integer) AS is_dry_spell, -- Cast to integer
